...
export const Cart = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
      ...
      <h1 className="font-serif text-3xl md:text-5xl text-gray-900 mb-2 italic">Shopping Cart</h1>
      <p className="text-sm uppercase tracking-widest text-gray-500 font-bold mb-12">Review your selections</p>
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center bg-gray-50 border border-gray-100 mt-8">
          ...
          <h2 className="font-serif text-2xl text-gray-900 mb-4">Your cart is empty</h2>
          <p className="text-sm text-gray-500 mb-8 max-w-sm leading-relaxed">Looks like you haven't added any terracotta art pieces to your cart yet.</p>
          <Link to="/shop">
            <Button className="uppercase text-xs tracking-widest px-8 py-3 h-auto">Explore Shop</Button>
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-16 items-start mt-8">
                ...
                <div key={item.cartItemId} className="grid grid-cols-12 gap-4 py-4 border-b border-gray-100 items-center">
                  <div className="col-span-12 md:col-span-6 flex gap-4">
                    ...
                    <div className="flex flex-col justify-center">
                      <Link to={`/product/${item.slug}`} className="font-serif text-lg text-gray-900 hover:text-terracotta transition-colors">{item.name}</Link>
                      <p className="text-terracotta font-medium text-sm mt-1">₹{item.price}</p>
          ...
          <div className="lg:col-span-1 border border-gray-200 bg-gray-50/50 p-6 sm:p-8 shrink-0 lg:sticky lg:top-32">
            ...
            <div className="space-y-4 text-sm text-gray-600 border-b border-gray-200 pb-6 mb-6">
              ...
              <div className="flex justify-between">
                <span>Shipping</span>
                <span className="text-terracotta font-medium tracking-wide text-xs uppercase pt-1">Calculated at checkout</span>
              </div>
            </div>
            <div className="flex justify-between items-center mb-8">
              <span className="font-serif text-xl text-gray-900">Total</span>
              <span className="font-serif text-2xl text-terracotta">₹{cartTotal()}</span>
            </div>
            
            <Link to="/checkout" className="block">
              <Button fullWidth className="h-14 uppercase text-xs tracking-widest bg-gray-900 flex items-center justify-center gap-2">
